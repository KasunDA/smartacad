...
class ClassStudentsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.master-records.classes.class-rooms.class-student',
    ...
    public function searchStudents(Request $request)
    {
        ...
        $students = StudentClass::where('academic_year_id', $inputs['student_academic_year_id'])
            ->where('classroom_id', $inputs['student_classroom_id'])
            ->get();
        ...
        if ($students->count() > 0) {
            $studentsNoClass = Student::where('status_id', 1)
                ->whereNotIn('student_id',
                    StudentClass::where('academic_year_id', $inputs['student_academic_year_id'])
                    ->pluck('student_id')
                    ->toArray()
                )
                ->get();
    ...
    public function validateClone(Request $request)
    {
        ...
        $from = StudentClass::where('academic_year_id', $from_year->academic_year_id)
            ->where('classroom_id', $inputs['from_classroom_id'])
            ->count();
        $to = StudentClass::where('academic_year_id', $to_year->academic_year_id)
            ->where('classroom_id', $inputs['to_classroom_id'])
            ->count();
    ...
    public function cloning(Request $request)
    {
        ...
        $from = StudentClass::where('academic_year_id', $inputs['from_year'])
            ->where('classroom_id', $inputs['from_class'])
            ->get();
        ...
        if ($from->count() > 0) {
            ...
            $this->setFlashMessage(' Cloned!!! ' . $from->count() .
                ' Students Class Rooms have been cloned from '.$from_year->academic_year.' to ' . $to_year->academic_year, 1
            );
        }
    }
}
