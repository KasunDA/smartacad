...
class BillingsController extends Controller
{
     ...
     */
    public function initiateBillings(Request $request) {
        $inputs = $request->all();
    ...
    public function searchResults(Request $request)
    {
        ...
        $inputs = $request->all();
        $students = $classrooms = $output = $items = $response = [];
        ...
        if (!empty($inputs['view_classroom_id'])) {
            ...
            $classrooms = ClassRoom::where('classlevel_id', $inputs['view_classlevel_id'])
                ->get();
        }
     ...
     * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
     */
    public function itemVariables(Request $request)
    {
        ...
        $ids = !empty($inputs['ids'] && strlen($inputs['ids']) > 1) 
            ? substr($inputs['ids'], 0, strlen($inputs['ids']) -1) 
            : $inputs['ids'];
     ...
     * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
     */
    public function itemQuotes(Request $request)
    {
        $inputs = $request->all();
        $term = AcademicTerm::findOrFail($inputs['term_id']);
