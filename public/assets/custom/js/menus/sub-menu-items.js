...
$(function () {
    $('.add_sub_menu_item').click(function(e){
        e.preventDefault();
        var clone_row = $('#sub_menu_item_table tbody tr:last-child').clone();
        var new_role = $('#new_roles').clone();

        $('#sub_menu_item_table tbody').append(clone_row);
        var count = $('#sub_menu_item_table tbody tr').length;
    ...
    $(document.body).on('click', '.delete_sub_menu_item',function(e){
        ...
        bootbox.dialog({
            message: "Are You sure You want to permanently delete sub menu item "+sub_menu_item,
            title: "Warning Alert",
            buttons: {
                danger: {
                    label: "NO",
                ...
                success: {
                    ...
                    callback: function() {
                        $.ajax({
                            type: 'GET',
                            async: true,
                            url: '/sub-menu-items/delete/' + sub_menu_item_id,
