...
$(function () {
    $('.add_grade').click(function(e){
        ...
        clone_row.children(':last-child').html('<button class="btn btn-danger btn-rounded btn-condensed btn-sm remove_grade"><span class="fa fa-times"></span> Remove</button>');
    });
    ...
    $(document.body).on('click', '.delete_grade',function(e){
        e.preventDefault();

        var parent = $(this).parent().parent();
        var grade = parent.children(':nth-child(2)').children('input').val();
        var grade_id = parent.children(':nth-child(2)').children('input[type=hidden]').val();

        bootbox.dialog({
            message: "Are You sure You want to permanently delete Grade: "+grade,
            title: "Warning Alert",
            buttons: {
                danger: {
                    label: "NO",
